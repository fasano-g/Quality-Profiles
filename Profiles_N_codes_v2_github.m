...
% -----------------------------------------------------------------
% Matrix used to plot Quality Profiles and Test Set Profiles. The columns
% of TT are (respectively): 
...
% This data will be uniquely used to plot Quality Profiles and Test Set 
...
% ------------------
    ...
    % Perform the other cycles of bootstrapping
    for c=2:cycles
        % fprintf('  Cycle = %i / %i  \n',c,cycles);
        ...
        for s=1:S
            for t=1:(intervals+1)
                ...
                counter = 0;
                for w=1:P
                    if ((TT_work(w,1+s) - f_star_w(w)) <= tau(t)*(f_0(w) - f_star_w(w)))
                        counter = counter + 1;
                    end
                end
                % Scaling, through the parameter 'r2', of the Quality Profiles 
...
% -----------------------
    ...
    figure3 = figure;   % PLOT of the QUALITY PROFILE
      ...
      figure4 = figure;         % PLOT of the TEST SET PROFILE
        ...
        if (logscaling)
            for s=1:S
                semilogx(tau(1:(intervals+1)),QQ_var(:,s),':','Color',colors(s,:),'LineWidth',2.5);
                %loglog(tau(1:(intervals+1)),QQ_var(:,s),':','Color',colors(s,:),'LineWidth',2.5);
                hold on
            end
        else
            for s=1:S
        ...
        if (strong)
            title([' {\color{red}Variance (Strong) Quality Profiles:} |S| = ', ...
                            num2str(S),';  |P| = ', ...
                            num2str(P),';  x-scaling (r_1 = ',num2str(r1), ...
                   ')'],'FontName','Baskerville Old Face', ...
                   'Fontsize',35);
        else
            title([' {\color{red}Variance (Weak) Quality Profiles:} |S| = ', ...
                            num2str(S),';  |P| = ', ...
                            num2str(P),';  x-scaling (r_1 = ',num2str(r1), ...
                   ')'],'FontName','Baskerville Old Face', ...
                   'Fontsize',35);
        ...
        grid on
        xlabel('0 \leq \tau^{r_1} \leq 1','FontWeight','bold','FontSize',35); 
        ylabel(' Var(\tau)','FontWeight','bold','FontSize',35);
        set(gca, 'FontSize', 15);
        % Here you can specify the name of the solvers in the 
        % vector "SolversNames": the following example is provided
        SolversNames = strseq(' Var\_Solver - ',1:S);
